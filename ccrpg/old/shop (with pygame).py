...
def button(msg, x, y, w, h, inactive_colour, active_colour, text_colour, name_of_function_to_call_when_clicked,
    ...
    if x + w > mouse[0] > x and y + h > mouse[1] > y:  # check if mouse is on button
        ...
        if click[0] == 1:  # check click (above if checks mouse is on button)
            name_of_function_to_call_when_clicked(
                veriable_in_function)  # do this when clicked (veriable needs not to have brackets)
    else:
...
def money_reset(s_money_new, m_money_new, l_money_new, x_money_new, money_original):
    ...
    money_original.append(s_money_new)
    money_original.append(m_money_new)
    money_original.append(l_money_new)
    money_original.append(x_money_new)
...
def equip_item(obj, name, player_equip):
    ...
    if obj == 0:
        ...
        saves_file.write(str(player_equip))
        saves_file.close()
        return weapons
    elif obj == 1:
        armour = name
            ...
            while decide == 0:
                for _ in pygame.event.get():
                    while length1 < len(inventry):
                        ...
                        button("Item number " + str(length1) + " is " + inventry[length1][4], 150, y, 500, 50, GREEN,
                               DARKGREEN, BLACK, item_to_eqip, length1)
                        pygame.display.flip()
                        length1 = length1 + 1
                        y = y + 50
                    length1 = 0
...
def save_item(to_write):
    ...
    amount_to_save = int(file.readline())
    amount_file.close()
...
warrior_medium_armour = [0, 45, 0, 0, "medium warrior armour", 1, 1, "better armour for the more experienced", 0, 45, 0,
                         0, 0]
...
mage_medium_armour = [0, 30, 0, 15, "medium mage armour", 2, 1, "better armour for the more experienced", 0, 45, 0, 0,
                      0]
...
paladin_medium_armour = [0, 65, 0, 0, "medium paladin armour", 3, 1, "better armour for the more experienced", 0, 45, 0,
                         0, 0]
...
paladin_ultimate_armour = [0, 600, 0, 0, "golden armour", 3, 1,
                           "not really made of gold, that would be expensive an impractical", 0, 0, 0, 5, 0]

necromancer_basic_armour = [0, 25, 0, 5, "basic necromancer armour", 4, 1, "basic armour for beginners", 0, 30, 0, 0, 0]
necromancer_medium_armour = [0, 40, 0, 5, "medium necromancer armour", 4, 1, "better armour for the more experienced",
                             0, 45, 0, 0, 0]
necromancer_strong_armour = [0, 55, 0, 15, "strong necromancer armour", 4, 1, "very good quality armour", 0, 70, 0, 0,
                             0]
necromancer_elite_armour = [0, 75, 0, 25, "elite necromancer armour", 4, 1, "elite armour of a gret quality", 0, 0, 10,
                            0, 0]
necromancer_ultimate_armour = [0, 350, 0, 150, "reaper's cloak", 4, 1, "the cloak of the lord of the dead", 0, 0, 0, 5,
                               0]

barbarian_basic_armour = [0, 20, 0, 0, "basic barbarian armour", 5, 1, "basic armour for beginners", 0, 30, 0, 0, 0]
barbarian_medium_armour = [0, 35, 0, 0, "medium barbarian armour", 5, 1, "better armour for the more experienced", 0,
                           45, 0, 0, 0]
barbarian_strong_armour = [0, 50, 0, 0, "strong barbarian armour", 5, 1, "very good quality armour", 0, 70, 0, 0, 0]
barbarian_elite_armour = [0, 70, 0, 0, "elite barbarian armour", 5, 1, "elite armour of a great quality", 0, 0, 10, 0,
                          0]
barbarian_ultimate_armour = [0, 400, 0, 0, "enraging armour", 5, 1, "a piece of armour that seems to enrage the wearer",
                             0, 0, 10, 0, 0]

lancer_basic_armour = [0, 10, 0, 0, "basic lancer armour", 6, 1, "basic armour for beginners", 0, 30, 0, 0, 0]
lancer_medium_armour = [0, 25, 0, 0, "medium lancer armour", 6, 1, "better armour for the more experienced", 0, 45, 0,
                        0, 0]
...
lancer_ultimate_armour = [0, 300, 0, 0, "lancelord armor", 6, 1, "ancient lancer armour imbued with fire", 0, 0, 0, 5,
                          0]

archer_basic_armour = [0, 25, 5, 0, "basic archer armour", 7, 1, "basic armour for beginners", 0, 30, 0, 0, 0]
archer_medium_armour = [0, 40, 10, 0, "medium archer armour", 7, 1, "better armour for the more expeienced", 0, 45, 0,
                        0, 0]
...
archer_ultimate_armour = [0, 400, 100, 0, "armour of retribution", 7, 1, "will have your enemies quivering with fear!",
                          0, 0, 0, 5, 0]

samurai_basic_armour = [0, 25, 0, 0, "basic samurai armour", 8, 1, "basic armour for beginnners", 0, 30, 0, 0, 0]
samurai_medium_armour = [0, 35, 0, 0, "medium samurai armour", 8, 1, "better armour for the more experienced", 0, 45, 0,
                         0, 0]
...
samurai_ultimate_armour = [100, 200, 100, 0, "crimson winged armour", 8, 1,
                           "winged armour of crimson, that once belonged to a legendary shogun", 0, 0, 0, 5, 0]

ninja_basic_armour = [0, 20, 5, 0, "basic ninja armour", 9, 1, "basic armour for beginners", 0, 30, 0, 0, 0]
ninja_medium_armour = [0, 30, 10, 0, "medium ninja armour", 9, 1, "better armour for the more experienced", 0, 45, 0, 0,
                       0]
...
warrior_medium_weapon = [45, 0, 0, 0, "medium warrior sword", 1, 0, "a better sword for the more experienced", 0, 45, 0,
                         0, 0]
...
mage_ultimate_weapon = [0, 0, 0, 500, "darkness staff", 2, 0, "the power to command dark matter is pretty cool", 0, 0,
                        0, 5, 0]

paladin_basic_weapon = [10, 0, 0, 0, "basic paladin hammer", 3, 0, "a basic hammer for beginners", 0, 30, 0, 0, 0]
paladin_medium_weapon = [25, 0, 0, 0, "medium paladin hammer", 3, 0, "a better hammer for the more experienced", 0, 45,
                         0, 0, 0]
...
necromancer_medium_weapon = [0, 10, 0, 35, "medium necromancer staff", 4, 0, "a better staff for the more experienced",
                             0, 45, 0, 0, 0]
necromancer_strong_weapon = [0, 15, 0, 55, "strong necromancer staff", 4, 0, "a very good quality staff", 0, 70, 0, 0,
                             0]
necromancer_elite_weapon = [0, 20, 0, 80, "elite necromancer staff", 4, 0, "an elite staff of a great quality", 0, 0,
                            10, 0, 0]
necromancer_ultimate_weapon = [0, 100, 0, 400, "demon scythe", 4, 0,
                               "a weapon that holds the secret to summoning chaos demons", 0, 0, 0, 5, 0]

barbarian_basic_weapon = [40, 0, 0, 0, "basic barbarian axe", 5, 0, "a basic axe for beginners", 0, 30, 0, 0, 0]
barbarian_medium_weapon = [55, 0, 0, 0, "medium barbarian axe", 5, 0, "a better axe for the more experienced", 0, 45, 0,
                           0, 0]
...
barbarian_ultimate_weapon = [600, 0, 0, 0, "axe of the dragon king", 5, 0, "an ancient axe from ages past", 0, 0, 0, 5,
                             0]

lancer_basic_weapon = [30, 0, 20, 0, "basic lancer lance", 6, 0, "a basic lance for beginners", 0, 30, 0, 0, 0]
lancer_medium_weapon = [40, 0, 25, 0, "medium lancer lance", 6, 0, "a better lance for the more experienced", 0, 45, 0,
                        0, 0]
...
lancer_ultimate_weapon = [500, 0, 200, 0, "spear of the piercer", 6, 0, "belonged to a titan in ages past", 0, 0, 0, 5,
                          0]

archer_basic_weapon = [10, 5, 15, 0, "basic archer bow", 7, 0, "a basic bow for beginners", 0, 30, 0, 0, 0]
archer_medium_weapon = [15, 10, 20, 0, "medium archer bow", 7, 0, "a better bow for the more experienced", 0, 45, 0, 0,
                        0]
...
archer_ultimate_weapon = [150, 125, 225, 0, "elven bow", 7, 0, "always the best bows in any movie or game", 0, 0, 0, 5,
                          0]

samurai_basic_weapon = [20, 0, 15, 0, "basic samurai katana", 8, 0, "a basic katana for beginners", 0, 30, 0, 0, 0]
samurai_medium_weapon = [30, 0, 25, 0, "medium samurai katana", 8, 0, "a better katana for the more experienced", 0, 45,
                         0, 0, 0]
samurai_strong_weapon = [50, 0, 40, 0, "strong samurai katana", 8, 0, "a very good quality katana", 0, 70, 0, 0, 0]
samurai_elite_weapon = [70, 0, 60, 0, "elite samurai katana", 8, 0, "an elite katana of a great quality", 0, 0, 10, 0,
                        0]
samurai_ultimate_weapon = [300, 0, 300, 0, "two handed katana", 8, 0,
                           "a katana that is specially designed to be wielded with both hands", 0, 0, 0, 5, 0]

ninja_basic_weapon = [10, 0, 35, 0, "basic ninja kama", 9, 0, "a basic kama for beginners", 0, 30, 0, 0, 0]
ninja_medium_weapon = [20, 0, 30, 0, "medium ninja kama", 9, 0, "a better kama for the more experienced", 0, 45, 0, 0,
                       0]
...
warrior_enchanted_armour = [50, 100, 0, 0, "enchanted armour", 1, 1, "armour that stops a lot of damage", 0, 150, 0, 0,
                            0]
...
warrior_old_iron_armour = [0, 100, 0, 0, "old iron armour", 1, 1, "armour taken from an ancient kingdom", 0, 100, 0, 0,
                           0]

mage_magic_armour = [0, 200, 0, 0, "magic armour", 2, 1, "enchanted armour designed to stop most damage", 0, 0, 20, 0,
                     0]
mage_power_armour = [0, 300, 0, 100, "one shot armour", 2, 1, "armoua that helps increase magic power", 0, 0, 40, 0, 0]
mage_dex_armour = [0, 100, 100, 100, "magic armour of dexterity", 2, 1, "armour that supports magic and speed", 0, 0,
                   30, 0, 0]
mage_oneshotarmour = [0, 0, 0, 750, "one shot armour", 2, 1, "armour designed to help kill enemies quickly", 0, 0, 75,
                      0, 0]
mage_chaos_armour = [0, 60, 0, 60, "chaos armour", 2, 1,
                     "armour of an ancient sorcerer, but most of it's power has faded", 0, 0, 120, 0, 0]

paladin_grace_armour = [25, 75, 100, 0, "armour of grace", 3, 1,
                        "paladin armour that supports fast movement and graceful strikes", 0, 0, 20, 0, 0]
paladin_blessed_armour = [150, 150, 100, 0, "armour of the blessed", 3, 1,
                          "armour that supports heavy defense and strikes", 0, 0, 40, 0, 0]
...
necromancer_dark_cloak = [0, 100, 200, 100, "dark cloak", 4, 1,
                          "a lightweight shroud that allows the wearer to avoid attacks", 0, 0, 20, 0, 0]
necromancer_wraith_armour = [100, 200, 0, 100, "wraith armour", 4, 1, "extremly powerful armour of the dark lords", 0,
                             0, 40, 0, 0]
necromancer_black_armour = [0, 500, 0, 0, "black armour", 4, 1, "armour to nullify most damage", 0, 0, 0, 5, 0]
necromancer_black_steel_armour = [0, 80, 0, 0, "black steel armour", 4, 1, "simple steel armour for necromancers", 0,
                                  80, 0, 0, 0]
necromancer_ghost_armour = [0, 30, 0, 0, "ghost armour", 4, 1, "armour of a ghost, that isn't too useful", 0, 30, 0, 0,
                            0]

barbarian_endurance_armour = [50, 150, 0, 0, "endurance armour", 5, 1, "for the fighter who wishes to survive", 0, 0,
                              20, 0, 0]
...
barbarian_white_steel_armour = [20, 80, 0, 0, "white steel armour", 5, 1, "simple armour for barbarians", 0, 100, 0, 0,
                                0]
...
lancer_protection_armour = [100, 200, 100, 0, "protecting armour", 6, 1, "for the lancer who doesn't wish to die", 0, 0,
                            40, 0, 0]
lancer_woven_armour = [20, 20, 20, 0, "woven armour", 6, 1, "armour made of leather woven with steel", 0, 60, 0, 0, 0]
lancer_ice_armour = [0, 200, 0, 0, "ice armour", 6, 1, "good solid armour made of ice (but useless against fire)", 0, 0,
                     20, 0, 0]
lancer_white_iron_armour = [0, 250, 50, 0, "white iron armour", 6, 1, "white armour is rare, so this armour is too", 0,
                            300, 0, 0, 0]

archer_frost_armour = [25, 150, 25, 0, "frost armour", 7, 1,
                       "armour that provides a shield of unbreaking ice for it's wearer", 0, 0, 20, 0, 0]
archer_agility_armour = [0, 0, 0, 400, "armour of agility", 7, 1,
                         "armour for the archer who wishes to inflict critical damage", 0, 0, 40, 0, 0]
archer_ash_armour = [0, 20, 20, 0, "ash armour", 7, 1, "tattered armour of one who escaped a volcano", 0, 40, 0, 0, 0]
archer_frost_armour = [0, 100, 0, 900, "frost armour", 7, 1,
                       "this armour is imbued with magical frost with a strange power", 0, 0, 0, 10, 0]
archer_black_leather_armour = [0, 150, 150, 0, "black leather armour", 7, 1, "designed for thieves, loved by archers",
                               0, 0, 30, 0, 0]

samurai_daredevil_armour = [600, 0, 100, 0, "daredevil armour", 8, 1,
                            "armour the imbues the wearer with an ancient madness", 0, 0, 0, 8, 0]
samurai_sakaretsu_armour = [50, 30, 20, 0, "sakaretsu armour", 8, 1, "armour that increases offensive capability", 0, 0,
                            10, 0, 0]
samurai_emperor_armour = [0, 80, 0, 0, "ermour of an emperor", 8, 1,
                          "armour of an emperor that united an ancient kingdom, designed purely for personal defence",
                          0, 80, 0, 0, 0]
samurai_ronin_armour = [50, 30, 0, 0, "ronin armour", 8, 1,
                        "armour of an outcast samurai, this armour is designed for offensive power", 0, 80, 0, 0, 0]
samurai_ebon_armour = [0, 50, 50, 0, "purple armour", 8, 1,
                       "an ancient set of purple armour that once belonged to a strong samurai", 0, 0, 10, 0, 0]
samurai_blue_armour = [50, 0, 50, 0, "blue armour", 8, 1,
                       "an ancient set of blue armour that once belonged to an agile samurai", 0, 0, 10, 0, 0]
samurai_orange_armour = [100, 0, 0, 0, "orange armour", 8, 1,
                         "an ancient set of orange armour that once belonged to a dangerous samurai", 0, 0, 10, 0, 0]
samurai_green_armour = [10, 0, 90, 0, "green armour", 8, 1,
                        "an ancient set of green armour that once belonged to a skilled samurai", 0, 0, 10, 0, 0]
samurai_yellow_armour = [55, 0, 45, 0, "yellow armour", 8, 1,
...
warrior_balanced_sword = [30, 30, 30, 30, "balanced blade", 1, 0,
                          "this blade is made with perfect balance in all things", 0, 120, 0, 0, 0]
warrior_goddess_sword = [35, 15, 0, 0, "goddess blade", 1, 0,
                         "a sword of a goddess who sacrificed her powers to become human", 0, 50, 0, 0, 0]
warrior_sword_of_winds = [15, 35, 0, 0, "sword of winds", 1, 0, "an ancient sword with a faint airy power", 0, 50, 0, 0,
                          0]
warrior_sword_of_ages = [15, 0, 0, 35, "sword of ages", 1, 0, "a sword from a land warped by time", 0, 50, 0, 0, 0, ]
warrior_silver_sword = [1000, 0, 1000, 0, "silver sword", 1, 0,
                        "a sword that once belonged to a knight who could defend his city against anyone", 0, 0, 0, 20,
                        0]

mage_weaving_wand = [70, 0, 30, 100, "weaving wand", 2, 0, "for the mage who wishes to weave in attacks between spells",
                     0, 0, 20, 0, 0]
mage_dragon_wand = [0, 0, 100, 300, "dragon wand", 2, 0, "a wand made from dragon bones", 0, 0, 40, 0, 0]
mage_serpent_wand = [0, 0, 200, 500, "serpent wand", 2, 0,
                     "a wand the fires spells as fast as a viper, and has the deadliness to match", 1, 0, 70, 0, 0]
mage_one_shot_wand = [0, 0, 0, 750, "one shot wand", 2, 0, "a wand that is designed to kill an enemy quickly", 0, 0, 75,
                      0, 0]
...
paladin_lightbringer = [200, 100, 100, 0, "the lightbringer", 3, 0,
                        "a sword imbued with a holy light that rips through enemies", 0, 0, 40, 0, 0]
paladin_hammer_of_dawn = [40, 0, 0, 0, "the hammer of dawn", 3, 0,
                          "this hammer hits like the first rays of dawn, on a planet with 3 suns", 0, 0, 4, 0, 0]
paladin_holy_axe = [100, 0, 0, 0, "holy axe", 3, 0, "an axe imbued with holy light", 0, 0, 10, 0, 0]
paladin_life_hammer = [200, 200, 0, 0, "life hammer", 3, 0,
                       "the power of this hammer can bring poeple back from the dead", 0, 0, 0, 4, 0]
...
necromancer_dark_shield = [0, 150, 0, 50, "dark shield", 4, 0, "for the necromancer who wishes to be hard to kill", 0,
                           0, 20, 0, 0]
necromancer_shadow_blade = [200, 100, 0, 100, "shadow blade", 4, 0, "a blade tainted by darkness", 0, 0, 40, 0, 0]
necromancer_crystal_ball = [0, 50, 0, 50, "crystal ball", 4, 0, "a simple crystal ball that increases magic power", 0,
                            0, 10, 0, 0]
necromancer_black_staff = [0, 0, 0, 150, "black staff", 4, 0, "a necromancer staff made of black iron", 0, 0, 15, 0, 0]
necromancer_dimension_blade = [35, 45, 0, 0, "dimension blade", 4, 0,
                               "a sword from a different dimension with an eerie power", 0, 80, 0, 0, 0]
necromancer_bone_staff = [0, 0, 0, 200, "bone staff", 4, 0, "a staff made from a demon's bone", 0, 0, 20, 0, 0]
necomancer_night_staff = [0, 1000, 0, 2000, "night staff", 4, 0, "a staff only for the best of necromancers", 0, 0, 0,
                          30, 0]

barbarian_dragon_sword = [50, 50, 100, 0, "dragon sword", 5, 0, "a lightweight sword taken from a dragon's treasure", 0,
                          0, 20, 0, 0]
...
barbarian_hammer = [600, 0, -100, 0, "barbarian's hammer", 5, 0, "a hammer large and heavy enough to crush foes", 0, 0,
                    50, 0, 0]
barbarian_dragon_blade = [2000, 0, 2000, 0, "dragonslayer blade", 5, 0,
                          "a curved sword of a barbarian who busted many a dragon", 0, 0, 0, 40, 0]

lancer_trident = [70, 70, 60, 0, "the trident", 6, 0, "protects the user with a shield of water", 1, 0, 20, 0, 0]
lancer_glaive = [200, 0, 200, 0, "the glaive", 6, 0, "a pole-arm designed to be slashed rather than thrusted", 1, 0, 40,
                 0, 0]
lancer_conquest_lance = [350, 300, 350, 0, "conquest lance", 6, 0, "lance of a rider who rode a white horse", 1, 0, 0,
                         10, 0]
lancer_war_lance = [500, 0, 500, 0, "war lance", 6, 0, "lance of a rider who rode a red horse", 1, 0, 0, 10, 0]
lancer_famine_lance = [250, 500, 250, 0, "famine lance", 6, 0, "lance of a rider who rode a black horse", 1, 0, 0, 10,
                       0]
lancer_death_lance = [1000, 0, 0, 0, "death lance", 6, 0, "lance of a rider who rode a pale green horse", 1, 0, 0, 10,
                      0]
lancer_oni_lance = [4000, 0, 1000, 0, "oni's lance", 6, 0,
                    "the lance of one of four riders, but not the four you're thinking of", 1, 0, 0, 50, 0]

archer_crossbow = [200, 0, 0, 0, "crossbow", 7, 0, "similar to a bow, but more mechanical and requires less skill", 1,
                   0, 20, 0, 0]
archer_hermes_bow = [50, -20, 30, 0, "bow of hermes", 7, 0, "a bow that delivers a message, a painful message", 1, 0,
                     40, 0, 0]
archer_explosive_bow = [500, 0, 60, 0, "explosive bow", 7, 0,
                        "a bow that fires arrows that explode upon hitting their target", 1, 0, 56, 0, 0]
archer_black_bow = [100, 0, 700, 0, "black bow", 7, 0,
                    "a bow made of a very srong wood that lets it hit from miles away", 1, 0, 80, 0, 0]
archer_greatbow = [600, 0, 0, 0, "greatbow", 7, 0,
                   "an extremly heavy bow that fires extremly heavy arrows that pack a huge punch", 1, 0, 60, 0, 0]
archer_shotgun = [2000, 0, 0, 0, "the shotgun", 7, 0,
                  "not really a shotgun, just a crossbow that fires 6 bolts at once", 1, 0, 0, 20, 0]
...
samurai_aluminium_katana = [1000, 500, 1500, 0, "aluminium katana", 8, 0,
                            "a katana made from magically enhanced aluminium, in respect to a famed samurai", 0, 0, 0,
                            30, 0]
...
ninja_extended_bo_staff = [400, 0, 100, 0, "extended bo staff", 9, 0, "a staff that is good at range but bad up close",
                           1, 0, 50, 0, 0]
...
# weapons that multiple classes can use (future implimentation?)
...
berserkers_band = [250, -50, 0, 0, "Beserkers band of destruction", 0, 2,
                   "A long dead berserkers band that was enchanted with the power of satan", 0, 0, 0, 0, 1]
priest_band = [0, 100, 0, 50, "Priests saintly miracle band", 0, 2, "A saintly clerics lost wedding band", 0, 0, 0, 0,
               1]
# this needs to heal you for 25
Fire_gem_circlet = [0, 0, 50, 150, "Fire gem circlet", 0, 2,
                    "A gem formed in the fires of hell mounted onto a malachite circlet", 0, 0, 0, 0, 1]
major_ring = [50, 50, 50, 50, "Major ring of stat boosting", 0, 2, "A ring to make your power level over 9000", 0, 0, 0,
              0, 1]
ring_of_random_change = [0, 0, 0, 0, "Ring of random change", 0, 2,
                         "A ring that causes abnormalities in the atmosphere like hats coming out of rabbits", 0, 0, 0,
                         0, 1]
# should do random things in battle like kill you or the enemy instantly
blinding_cranium_crab = [0, 0, 0, 0, "Blinding cranium crab", 0, 2,
                         "A crab for your cranium that jumps and blinds your foe lowering dexterity", 0, 0, 0, 0, 1]
# should lower enemy dexterity
swiss_army_claymore = [50, 0, 50, 0, "Swiss army claymore", 0, 2,
                       "A fold out sword application for a strange swiss army knife", 0, 0, 0, 0, 1]
# needs to make you go second every second turn and then give a strength boost of 200
arrow_target = [0, 0, 100, 0, "Archery circlet of the pure archer", 0, 2,
                "An elven kings son once owned this crown and went on to defeat a dark lord", 0, 0, 0, 0, 1]
# Should give a random chance to mean that all attacks by the player that turn become critical
blight_sludge = [0, 0, 0, 0, "Blight sludge", 0, 2, "Some sludge taken from a poison riddled place that emanates death",
                 0, 0, 0, 0, 1]
# Should deal poison damage over time to the enemy
overpowered_stick = [1000, 1000, 1000, 1000, "The overpowered stick of doom", 0, 2,
                     "A stick blessed with the power of bad coding", 0, 0, 0, 0, 1]
# can only be used in one battle then it breaks and can't be used in boss battles
boss_shield = [0, 200, 0, 0, "Boss shield of death", 0, 2,
               "A shield that is so big we didn't want to code it so here's a buckler", 0, 0, 0, 0, 1]
...
items = [warrior_basic_armour, warrior_medium_armour, warrior_strong_armour, warrior_elite_armour,
         warrior_ultimate_armour, mage_basic_armour, mage_medium_armour, mage_strong_armour, mage_ultimate_armour,
         paladin_basic_armour, paladin_medium_armour, paladin_strong_armour, paladin_elite_armour,
         paladin_ultimate_armour, necromancer_basic_armour, necromancer_medium_armour, necromancer_strong_armour,
         necromancer_elite_armour, necromancer_ultimate_armour, barbarian_basic_armour, barbarian_medium_armour,
         barbarian_strong_armour, barbarian_elite_armour, barbarian_ultimate_armour, lancer_basic_armour,
         lancer_medium_armour, lancer_strong_armour, lancer_elite_armour, lancer_ultimate_armour, archer_basic_armour,
         archer_medium_armour, archer_strong_armour, archer_elite_armour, archer_ultimate_armour, warrior_basic_weapon,
         warrior_medium_weapon, warrior_strong_weapon, warrior_elite_weapon, warrior_ultimate_weapon, mage_basic_weapon,
         mage_medium_weapon, mage_strong_weapon, mage_elite_weapon, mage_ultimate_weapon]  # maximum defininf length
items_accsesorys = [berserkers_band, priest_band, Fire_gem_circlet, major_ring, ring_of_random_change,
                    blinding_cranium_crab, swiss_army_claymore, arrow_target, blight_sludge, overpowered_stick,
                    boss_shield, sleepy_stick, lol, necrotic_bone, mr_tiddles, paladin_basic_weapon,
                    paladin_medium_weapon, paladin_strong_weapon, paladin_elite_weapon, paladin_ultimate_weapon,
                    necromancer_basic_weapon, necromancer_medium_weapon, necromancer_strong_weapon,
                    necromancer_elite_weapon, necromancer_ultimate_weapon, barbarian_basic_weapon,
                    barbarian_medium_weapon, barbarian_strong_weapon, barbarian_elite_weapon, barbarian_ultimate_weapon,
                    lancer_basic_weapon, lancer_medium_weapon, lancer_strong_weapon, lancer_elite_weapon,
                    lancer_ultimate_weapon, archer_basic_weapon, archer_medium_weapon, archer_strong_weapon,
                    archer_elite_weapon, archer_ultimate_weapon, samurai_basic_weapon, samurai_medium_weapon,
                    samurai_strong_weapon, samurai_elite_weapon, samurai_ultimate_weapon, ninja_basic_weapon,
                    ninja_medium_weapon]  # adds to arary
items.extend(items_accsesorys)  # add accsesorys to end of items array
items1 = [ninja_strong_weapon, ninja_elite_weapon, ninja_ultimate_weapon, warrior_spiked_armour,
          warrior_enchanted_armour, warrior_stone_armour, warrior_simple_armour, warrior_old_iron_armour,
          mage_magic_armour, mage_power_armour, mage_dex_armour, mage_oneshotarmour, mage_chaos_armour,
          paladin_grace_armour, paladin_blessed_armour, paladin_the_wall, paladin_steel_armour, paladin_shield_armour,
          necromancer_dark_cloak, necromancer_wraith_armour, necromancer_black_armour, necromancer_black_steel_armour,
          necromancer_ghost_armour, barbarian_endurance_armour, barbarian_flame_armour, barbarian_hard_leather_armour,
          barbarian_white_steel_armour, barbarian_brass_armour, lancer_speed_armour, lancer_protection_armour,
          lancer_woven_armour, lancer_ice_armour, lancer_white_iron_armour, archer_frost_armour, archer_agility_armour,
          archer_ash_armour, archer_black_leather_armour, samurai_daredevil_armour, samurai_sakaretsu_armour,
          samurai_emperor_armour, samurai_ronin_armour, samurai_ebon_armour]  # add acsessorys to new array
items.extend(items1)
items2 = [samurai_blue_armour, samurai_orange_armour, samurai_yellow_armour, samurai_green_armour, samurai_red_armour,
          samurai_battle_worn_armour, ninja_light_dan_gi, ninja_light_kyu_gi, ninja_heavy_kyu_gi, ninja_heavy_dan_gi,
          ninja_darkness_armour, warrior_cleaver, warrior_voltsword, warrior_balanced_sword, warrior_goddess_sword,
          warrior_sword_of_winds, warrior_sword_of_ages, warrior_silver_sword, mage_weaving_wand, mage_dragon_wand,
          mage_serpent_wand, mage_one_shot_wand, mage_twilight_wand, mage_corrupted_wand, mage_apocalypse_wand,
          paladin_light_hammer, paladin_lightbringer, paladin_hammer_of_dawn, paladin_holy_axe, paladin_life_hammer,
          paladin_heavy_hammer, paladin_hammer_of_judgement, necromancer_dark_shield, necromancer_shadow_blade,
          necromancer_crystal_ball, necromancer_black_staff, necromancer_dimension_blade, necromancer_bone_staff,
          necomancer_night_staff, barbarian_dragon_sword, barbarian_bloodaxe, barbarian_battle_axe, barbarian_twin_axe,
          barbarian_hammer]  # adds items to the array
items.extend(items2)
items3 = [barbarian_dragon_blade, lancer_trident, lancer_glaive, lancer_conquest_lance, lancer_war_lance,
          lancer_famine_lance, lancer_death_lance, lancer_oni_lance, archer_crossbow, archer_hermes_bow,
          archer_explosive_bow, archer_black_bow, archer_greatbow, archer_shotgun, samurai_dark_katana,
          samurai_light_katana, samurai_gold_katana, samurai_scimitar, samurai_twinblade, samurai_wakizashi,
          samurai_aluminium_katana, ninja_jo_staff, ninja_bo_staff, ninja_extended_bo_staff, ninja_surikens,
          ninja_kunai, ninja_dagger, ninja_tashi, basic_crossbow, medium_crossbow, strong_crossbow, basic_halberd]
items.extend(items3)
...
while 1 > 0:
    ...
    if instruction == "\\money":
        ...
        while loop < length:
            ...
            print("Item " + str(loop) + " - " + items[loop][4] + " costs " + str(
                store[1][loop]) + " from obrb type " + str(store[2][loop]))
            loop = loop + 1
